...
const CurrentExerciseItem = (props) => {
	const { id, exercise, sets, reps, weights, onAdd, onRemove } = props;
	...
	return (
		<div className={styles['current-exercise-container']}>
			<div className={styles['exercise-positioner']}>
				...
				{onRemove && <h1 className={styles['exercise-on-remove']} onClick={() => onRemove(id)} >-</h1>}
			</div>
			...
			<div className={styles['exercise-positioner']}>
				...
				<div className={styles['set-reps-container']}>
					...
					{weights && weights > 0 && <p>Current Weight : {weights}</p>}
				</div>
			</div>
